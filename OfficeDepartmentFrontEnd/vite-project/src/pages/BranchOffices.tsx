...
export const BranchOffices = () => {
  ...
  const loadData = async () => {
    try {
      const officesData = await branchOfficeService.getAll();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingOffice) {
        await branchOfficeService.update(editingOffice.id, formData);
      } else {
        await branchOfficeService.create(formData);
      }
      setShowModal(false);
      ...
      alert(error.response?.data?.message || 'Ошибка при сохранении офиса');
    }
  };
    ...
    <Layout>
      <div className="px-4 py-6 sm:px-0">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-900">Офисы</h1>
          {isAdmin && (
            <button
              onClick={() => {
                setEditingOffice(null);
              ...
              <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <form onSubmit={handleSubmit} className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                  <h3 className="text-lg leading-6 font-medium text-gray-900 mb-4">
                    {editingOffice ? 'Редактировать офис' : 'Добавить офис'}
                  </h3>
